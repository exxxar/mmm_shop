  ...
  async _getSharedData(url = "/") {
    return this.request(url)
      .then((html) => {
        ...
        if (html) {
          const shared = html.split("window._sharedData = ");
          if (shared) {
            const sharedData = shared[1];
            if (sharedData) {
              const sharedArr = sharedData.split(";</script>");
              if (sharedArr && sharedArr[0]) {
                return sharedArr[0];
              }
            }
          }
      ...
      .then((_sharedData) => (_sharedData ? JSON.parse(_sharedData) : ""));
  }
